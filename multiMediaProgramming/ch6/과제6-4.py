...
class TrafficWeak(QMainWindow):
    def __init__(self):
        super().__init__()
        self.setWindowTitle('물건 감지')
        self.setGeometry(200, 200, 700, 200)

        signButton = QPushButton('화장품 등록', self)
        roadButton = QPushButton('물건 영상 불러옴', self)
        ...
        self.signFiles = [['sugar.jpg', '립밤'], ['sunstick.jpg', '썬스틱'], ['handcream.jpg', '핸드크림']]  # 표지판 모델 영상
        self.signImgs = []  # 표지판 모델 영상 저장

    def signFunction(self):
        self.label.clear()
        self.label.setText('화장품을 등록합니다.')

        for fname, _ in self.signFiles:
            self.signImgs.append(cv.imread(fname))
            cv.imshow(fname, cv.resize(self.signImgs[-1], dsize=(0, 0), fx=0.5, fy=0.5))

    def roadFunction(self):
        if self.signImgs == []:
            self.label.setText('먼저 화장품을 등록하세요.')
        else:
            fname = QFileDialog.getOpenFileName(self, '파일 읽기', './')
            self.roadImg = cv.imread(fname[0])
            if self.roadImg is None: sys.exit('파일을 찾을 수 없습니다.')

            cv.imshow('Road scene', cv.resize(self.roadImg, dsize=(0, 0), fx=0.5, fy=0.5))

    def recognitionFunction(self):
        if self.roadImg is None:
            self.label.setText('먼저 물건 영상을 입력하세요.')
        else:
            sift = cv.SIFT_create()
            ...
            if len(GM[best]) < 4:  # 최선의 번호판이 매칭 쌍 4개 미만이면 실패
                ...
                cv.imshow('Matches and Homography', cv.resize(img_match, dsize=(0, 0), fx=0.5, fy=0.5))

                self.label.setText(self.signFiles[best][1] + '입니다.')

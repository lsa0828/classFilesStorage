...
median = cv.mediaBlur(gray, 3)
_, gray_bin = cv.threshold(gray, 0, 255, cv.THRESH_BINARY_INV+cv.THRESH_OTSU)

cnt, labels, stats, centroids = cv.connectedComponentsWithStats(gray_bin)
# cv.connectedComponentsWithStats의 입력영상은 배겨은 black, 물체는 white인 이진 영상

dst = cv.cvtColor(gray, cv.COLOR_GRAY2BGR) # 색상 사각형을 그리기 위해 color로 변환
dst[labels == 0] = [127, 127, 127]
dst[labels == 0] = [127, 0, 0]
dst[labels == 0] = [0, 127, 0]
dst[labels == 0] = [0, 0, 127]
dst[labels == 0] = [0, 127, 127]

for i in range(1, cnt): # 각각의 객체 정보에 들어가기 위해 반복문. 범위를 1부터 시작한 이유는 배경을 제외
    (x, y, w, h, area) = stats[i]

    # 노이즈 제거
    if area < 20: # 크기가 작은 연결 요소는 무시, 사각형 그리지 않음
        continue
